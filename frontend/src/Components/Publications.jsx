...
export default function Publications({
  ...
  return (
    <section className="mb-12 relative">
      ...
      {mostReactionsPub && (
        <>
          ...
          <div className="mb-8 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
            ...
            >
              <div
                className={`h-2 ${getToneColor(
                  mostReactionsPub.tone
                )} transition-all duration-300 group-hover:h-3`}
              />
              ...
              <div className="p-4">
                <div className="flex justify-between mb-3">
                  <div>
                    ...
                    <div className="text-sm text-neutral-500">
                      par {mostReactionsPub.author} •{" "}
                      {mostReactionsPub.timestamp}
                    </div>
                  </div>

                  <span
                    className={`text-xl ${getToneColor(
                      mostReactionsPub.tone
                    )}/20 rounded-full w-9 h-9 flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
                  >
                    {mostReactionsPub.tone}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  >
                    Lire{" "}
                    <ArrowUpRight className="w-3 h-3 ml-1 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200" />
                  </Link>
                </div>
              </div>
            </div>
      ...
      <h2
        className={`text-xl font-bold my-5 text-start flex items-center gap-3 ${
          darkMode ? "text-white" : "text-neutral-800"
        }`}
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pages
          .filter(
            (page) => !mostReactionsPub || page.id !== mostReactionsPub.id
          )
          .slice(0, 3)
          .map((page) => (
            <div
