...
class CsrfExemptSessionAuthentication(SessionAuthentication):
    """Session authentication sans contrôle CSRF (API only)."""
...
class SignupStep1View(APIView):
    ...
    def post(self, request):
        ...
        print("SignupStep1 errors →", serializer.errors)
...
class SignupStep2View(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Utilisateur invalide ou déjà activé"},
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Inscription terminée"}, status=status.HTTP_200_OK
            )

        print("SignupStep2 errors →", serializer.errors)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
# Alternative pure Django si besoin
@csrf_exempt
@require_POST
def logout_plain(request):
    logout(request)
